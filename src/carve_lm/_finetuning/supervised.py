...
class SupervisedFineTuner:
    """Accelerate-based supervised fine-tuning trainer for causal language models."""
    ...
    def __init__(
        ...
        if self._tracking_enabled:
            self.accelerator.init_trackers(
                ...
                config=self.config,
                init_kwargs={"wandb": {"name": self.config.get("wandb_run_name", "supervised_finetune")}},
            )
    ...
    def train(self):
        ...
        self.accelerator.print("Supervised fine-tuning finished.")
        return self.history
    ...
    def save_model(self, output_dir):
        ...
        if self.accelerator.is_main_process:
            ...
            artifact = self.tokenizer
            if artifact is None and hasattr(unwrapped_model, "tokenizer"):
                artifact = unwrapped_model.tokenizer
            if artifact is None and hasattr(unwrapped_model, "processor"):
                artifact = unwrapped_model.processor
            if artifact is not None:
                artifact.save_pretrained(output_path)
            self.accelerator.print(f"Model and tokenizer/processor saved to {output_path}")


__all__ = ["SupervisedFineTuner"]
