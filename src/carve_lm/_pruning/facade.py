...
from .manifest import MANIFEST_FILENAME, build_manifest, load_pruned_result, save_pruned_result
...
class _BaseStructuredPruner:
    ...
    # Domain hooks — each domain sets these on its facade subclasses. The engine
    # never imports a concrete adapter resolver, estimator factory, or manifest
    # filename; the LLM and VLM shims inject them here.
    _resolve_model_adapter = None
    _estimate_scores = None
    ...
    def __init__(self, model, config, device: str = "cpu", model_adapter=None):
        ...
        self.config = config
        self.device = device
        self.adapter = type(self)._resolve_model_adapter(model, model_adapter)
    ...
    def _score(self, dataloader) -> dict[str, float]:
        return type(self)._estimate_scores(
            self.model,
            self.adapter,
            self._last_context,
    ...
    def save_pruned(self, output_dir, result: PruningResult | None = None):
        ...
        return save_pruned_result(output_dir, result, filename=type(self)._manifest_filename)

    @classmethod
    def load_pruned(cls, output_dir, device: str | None = None, dtype=None) -> PruningResult:
        return load_pruned_result(cls, output_dir, device=device, dtype=dtype, filename=cls._manifest_filename)
...
class WidthGroupPruner(_BaseStructuredPruner):
    ...
    def estimate(self, dataloader) -> dict[str, float]:
        if self._last_context is None:
            self.discover()
        self._last_scores = self._score(dataloader)
