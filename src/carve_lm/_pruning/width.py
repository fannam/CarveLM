"""
Shared element-level (Level-1) width pruner and pruning strategies.

Domain-agnostic: the LLM and VLM-language stacks register these classes into
their own ``PRUNER_REGISTRY`` / ``PRUNING_STRATEGY_REGISTRY`` and bind the
adapter resolver, strategy registry, and embedding-scoring helper through the
``WidthPruner`` class hooks.
"""
...
from .pruner_base import _BasePruner
...
class WidthPruner(_BasePruner):
    ...
    * embedding channels (hidden dimension)
    ...
    # Domain hooks — bound on the per-domain WidthPruner subclass.
    _strategy_registry = None
    _embedding_global_score = None
    ...
    def _normalize_strategy_name(self, strategy_name: str) -> str:
        registry = type(self)._strategy_registry
    ...
    def available_strategies(self) -> tuple[str, ...]:
        return tuple(
            name
            for name in type(self)._strategy_registry.names()
            if name.startswith("element.")
        )

    def apply(self, strategy_name: str, **kwargs):
        normalized_name = self._normalize_strategy_name(strategy_name)
        strategy_cls = type(self)._strategy_registry.get(normalized_name)
    ...
    def _global_embedding_scores(self, embedding_importance) -> torch.Tensor:
        if isinstance(embedding_importance, Mapping):
            return type(self)._embedding_global_score(embedding_importance)
