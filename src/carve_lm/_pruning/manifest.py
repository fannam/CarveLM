...
# Default manifest filename. Each domain passes its own name via the facade's
# ``_manifest_filename`` hook; this default only applies to direct callers.
MANIFEST_FILENAME = "pruner_manifest.json"
...
def save_pruned_result(output_dir, result: PruningResult, *, filename: str = MANIFEST_FILENAME) -> Path:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    torch.save(result.model.state_dict(), output_path / STATE_FILENAME)
    json_dump(output_path / filename, result.manifest)
    return output_path


def load_pruned_result(
    pruner_cls,
    output_dir,
    ...
    *,
    ...
    output_path = Path(output_dir)
    manifest = json_load(output_path / filename)
