...
from ._protocol import ModelAdapter as BaseModelAdapter


class _BasePruner:
    # Domain hook — each domain binds its own ``resolve_model_adapter`` on the
    # facade subclasses (see the per-domain ``_base.py`` shims).
